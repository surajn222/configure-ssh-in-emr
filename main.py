...
def ssh_and_update_cron(list_ips):
    ...
    for i in list_ips:
        ...
        if str(i) == "172.30.138.8":
            ...
            ssh_cron_command = 'ssh ec2-user@' + i + ' "sudo bash -c \'touch /etc/cron.d/datadog-metrics; echo \\\"* * * * * root /usr/bin/python /efs/users/datadog_agent-custom-metrics/main.py CLUSTERNAME / >> /tmp/datadog-metrics.log 2>&1\\\" >  /etc/cron.d/datadog-metrics; \'"'
